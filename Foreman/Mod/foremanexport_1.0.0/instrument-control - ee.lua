...
local function ExportLocalisedString(lstring)
	-- as could be expected if lstring doesnt have a working translation then we get a beauty of 
...
local function ExportResearch()
	...
	for name, tech in pairs(AvailableTech) do
		...
		ExportParameter('icon_name', 'icon.t.'..tech.name, false, ',', 3)
		ExportParameter('enabled', tech.enabled and 'true' or 'false', true, ',', 3)

		ExportLine('"localised_name": "', 3)
		ExportLocalisedString(tech.localised_name)
		ExportLine('",', 0)
...
local function ExportRecipes()
	...
	for name, recipe in pairs(AvailableRecipes) do
		...
		ExportParameter('icon_name', 'icon.r.'..recipe.name, false, ',', 3)

		if recipe.products[1] then
			ExportParameter('icon_alt_name', 'icon.i.'..recipe.products[1].name, false, ',', 3)
		else
			ExportParameter('icon_alt_name', 'icon.r.'..recipe.name, false, ',', 3) --just in case there is no products, we will just have the alt-icon also refer to the regular icon
		end
		ExportParameter('enabled', recipe.enabled and 'true' or 'false', true, ',', 3)

		ExportLine('"localised_name": "', 3)
		ExportLocalisedString(recipe.localised_name)
		ExportLine('",', 0)
...
local function ExportItems()
	...
	for name, item in pairs(AvailableItems) do
		...
		ExportParameter('icon_name', 'icon.i.'..item.name, false, ',', 3)

		ExportLine('"localised_name": "', 3)
		ExportLocalisedString(item.localised_name)
		ExportLine('",', 0)
...
local function ExportFluids()
	...
	for name, fluid in pairs(AvailableFluids) do
		...
		ExportParameter('icon_name', 'icon.i.'..fluid.name, false, ',', 3)

		ExportLine('"localised_name": "', 3)
		ExportLocalisedString(fluid.localised_name)
		ExportLine('",', 0)
...
local function ExportModules()
	...
	for name, module in pairs(AvailableModules) do
		...
		ExportParameter('icon_name', 'icon.i.'..module.name, false, ',', 3)

		ExportLine('"localised_name": "', 3)
		ExportLocalisedString(module.localised_name)
		ExportLine('",', 0)
...
local function ExportCraftingMachines()
	...
	for name, machine in pairs(AvailableAMachines) do
		...
		ExportParameter('icon_name', 'icon.i.'..machine.name, false, ',', 3)

		ExportLine('"localised_name": "', 3)
		ExportLocalisedString(machine.localised_name)
		ExportLine('",', 0)
...
local function ExportMiners()
	...
	for name, miner in pairs(AvailableMiners) do
		...
		ExportParameter('icon_name', 'icon.i.'..miner.name, false, ',', 3)

		ExportLine('"localised_name": "', 3)
		ExportLocalisedString(miner.localised_name)
		ExportLine('",', 0)
...
local function ExportResources()
	...
	for name, resource in pairs(AvailableResources) do
		...
		ExportLine('"localised_name": "', 3)
		ExportLocalisedString(resource.localised_name)
		ExportLine('",', 0)
...
local function ExportGroups()
	...
	for name, group in pairs(game.item_group_prototypes) do
		...
		ExportParameter('icon_name', 'icon.g.'..group.name, false, ',', 3)

		ExportLine('"localised_name": "', 3)
		ExportLocalisedString(group.localised_name)
		ExportLine('",', 0)
