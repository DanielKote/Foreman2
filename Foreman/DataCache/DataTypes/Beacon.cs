...
namespace Foreman {
    ...
    internal class BeaconPrototype : EntityObjectBasePrototype, Beacon {
        public double GetBeaconEffectivity(Quality quality, double beaconCount) {
            ...
            var baseCount = (int) Math.Truncate(beaconCount);
            var remainder = beaconCount - baseCount;

            var lowerBeaconEffectivity = baseCount * GetBeaconEffectivityBase(quality, baseCount);
            var upperBeaconEffectivity = (baseCount + 1) * GetBeaconEffectivityBase(quality, baseCount + 1);
        ...
        // off by 1 from factorio (or more akin same index as LUA starts from 0)
        // ==> profile[x] is the multiplier for x beacons (so profile[0] is multiplier for 0 beacons...)
        internal double[] profile { get; private set; }

        public BeaconPrototype(DataCache dCache, string name, string friendlyName, EnergySource source, bool isMissing = false) :
            base(dCache, name, friendlyName, EntityType.Beacon, source, isMissing) {
