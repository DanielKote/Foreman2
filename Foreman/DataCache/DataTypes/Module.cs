...
namespace Foreman {
    public interface Module : DataObjectBase {
        ...
        double GetPollutionBonus(Quality quality);
        ...
        double GetPollutionBonus(int qualityLevel = 0);
    ...
    public class ModulePrototype : DataObjectBasePrototype, Module {
        ...
        public double GetSpeedBonus(Quality quality) {
            return GetSpeedBonus(quality.Level);
        }

        public double GetProductivityBonus(Quality quality) {
            return GetProductivityBonus(quality.Level);
        }

        public double GetConsumptionBonus(Quality quality) {
            return GetConsumptionBonus(quality.Level);
        }

        public double GetPollutionBonus(Quality quality) {
            return GetPollutionBonus(quality.Level);
        }

        public double GetQualityBonus(Quality quality) {
            return GetQualityBonus(quality.Level);
        }

        public double GetSpeedBonus(int qualityLevel = 0) {
            return SpeedBonus <= 0 || qualityLevel == 0 ? SpeedBonus : Math.Truncate(SpeedBonus * (1 + qualityLevel * 0.3) * 100) / 100;
        }

        public double GetProductivityBonus(int qualityLevel = 0) {
            return ProductivityBonus <= 0 || qualityLevel == 0 ? ProductivityBonus : Math.Truncate(ProductivityBonus * (1 + qualityLevel * 0.3) * 100) / 100;
        }

        public double GetConsumptionBonus(int qualityLevel = 0) {
            return ConsumptionBonus >= 0 || qualityLevel == 0 ? ConsumptionBonus : Math.Truncate(ConsumptionBonus * (1 + qualityLevel * 0.3) * 100) / 100;
        }

        public double GetPollutionBonus(int qualityLevel = 0) {
            return PollutionBonus >= 0 || qualityLevel == 0 ? PollutionBonus : Math.Truncate(PollutionBonus * (1 + qualityLevel * 0.3) * 100) / 100;
        }

        public double GetQualityBonus(int qualityLevel = 0) {
            return QualityBonus <= 0 || qualityLevel == 0 ? QualityBonus : Math.Truncate(QualityBonus * (1 + qualityLevel * 0.3) * 100) / 100;
        }
