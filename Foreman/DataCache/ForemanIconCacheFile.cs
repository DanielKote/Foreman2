...
namespace Foreman {
    ...
    public static class ForemanIconCacheFile {
        ...
        public static async Task WriteAsync(string path, IReadOnlyDictionary<string, IconColorPair> icons, int compressionLevel, CancellationToken cancellationToken = default) {
            ...
            await using var output = new FileStream(path, FileMode.OpenOrCreate, FileAccess.Write, FileShare.None, bufferSize: 64 * 1024, useAsync: true);
            await WriteHeaderAsync(output, entryCount, uncompressedSize, compressedSize: 0, cancellationToken);

            long compressedPayloadStart = output.Position;
            await using (var compressionStream = new CompressionStream(output, compressionLevel, leaveOpen: true))
                await WritePayloadAsync(compressionStream, entries, cancellationToken);

            uint compressedSize = (uint)(output.Position - compressedPayloadStart);
            await PatchCompressedSizeAsync(output, compressedSize, cancellationToken);
        }
        ...
        public static Task<Dictionary<string, IconColorPair>> ReadAsync(
            ...
            Task.Run(async () => {
                await using var input = new FileStream(path, FileMode.Open, FileAccess.Read, FileShare.Read, bufferSize: 64 * 1024, useAsync: true);
                ...
                if (input.Position != expectedEndPosition)
                    throw new InvalidDataException("FOIC compressed payload size mismatch.");
        ...
        private static async Task WritePayloadAsync(Stream output, IReadOnlyList<(byte[] NameUtf8, int ColorArgb, byte[] PngBytes)> entries, CancellationToken cancellationToken) {
            await WriteInt32Async(output, entries.Count, cancellationToken);

            foreach (var (nameUtf8, colorArgb, pngBytes) in entries) {
                cancellationToken.ThrowIfCancellationRequested();
                await WriteUInt16Async(output, (ushort)nameUtf8.Length, cancellationToken);
                await output.WriteAsync(nameUtf8, cancellationToken);
                await WriteInt32Async(output, colorArgb, cancellationToken);
                await WriteInt32Async(output, pngBytes.Length, cancellationToken);
                if (pngBytes.Length > 0)
                    await output.WriteAsync(pngBytes, cancellationToken);
            }
        }
        ...
        private static async Task<Dictionary<string, IconColorPair>> ParsePayloadAsync(
            ...
            int payloadEntryCount = await ReadInt32Async(payload, cancellationToken);
            ...
            for (int i = 0; i < payloadEntryCount; i++) {
                ...
                int nameByteLength = await ReadUInt16Async(payload, cancellationToken);
                ...
                string name = await ReadUtf8StringAsync(payload, nameByteLength, cancellationToken);
                payloadBytesRead += nameByteLength;

                int colorArgb = await ReadInt32Async(payload, cancellationToken);
                payloadBytesRead += sizeof(int);

                int pngByteLength = await ReadInt32Async(payload, cancellationToken);
                payloadBytesRead += sizeof(int);
                ...
                Bitmap? icon = null;
                if (pngByteLength > 0) {
                    byte[] pngBytes = await ReadBytesAsync(payload, pngByteLength, cancellationToken);
            ...
            if (payloadBytesRead != expectedUncompressedSize)
        ...
        private static async Task WriteHeaderAsync(Stream output, uint entryCount, uint uncompressedSize, uint compressedSize, CancellationToken cancellationToken) {
            await output.WriteAsync(MagicBytes, cancellationToken);
            ...
            await output.WriteAsync(headerTail, cancellationToken);
        }
        ...
        private static async Task PatchCompressedSizeAsync(Stream output, uint compressedSize, CancellationToken cancellationToken) {
            ...
            await WriteUInt32Async(output, compressedSize, cancellationToken);
