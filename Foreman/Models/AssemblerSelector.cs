...
namespace Foreman {
    public class AssemblerSelector {
        ...
        public Style DefaultSelectionStyle { get; set; } = Style.WorstNonBurner;


        public Assembler GetAssembler(Recipe recipe) {
            return GetAssembler(recipe, DefaultSelectionStyle);
        }

        public Assembler GetAssembler(Recipe recipe, Style style) {
            return GetOrderedAssemblerList(recipe, style).First();
        }

        public List<Assembler> GetOrderedAssemblerList(Recipe recipe) {
            return GetOrderedAssemblerList(recipe, DefaultSelectionStyle);
        }
